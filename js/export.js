/* ============================================
   GRID — Export Module
   PDF export with GRID branding
   ============================================ */

const GridExport = (() => {

  function generatePDF(note, renderedHTML) {
    ...
    container.innerHTML = `
      ...
      <div class="pdf-header">
        ...
        <span class="pdf-brand">GRID — Grade Research & Interactive Diary</span>
      </div>

      ${note.color ? `<div class="pdf-color-bar" style="background: var(--grid-label-${note.color})"></div>` : ''}

      <div class="pdf-breadcrumb">${note.subject} / ${note.chapter}</div>
      <h1 class="pdf-title">${note.title}</h1>

      <div class="pdf-meta">
        ${note.created ? `<span>Created: ${new Date(note.created).toLocaleDateString('en-IN')}</span>` : ''}
        ${note.updated ? `<span>Updated: ${new Date(note.updated).toLocaleDateString('en-IN')}</span>` : ''}
        <span>${GridRenderer.wordCount(note.content)} words</span>
      </div>

      ${note.tags && note.tags.length ? `
        <div class="pdf-tags">
          ${note.tags.map(t => `<span class="pdf-tag">${t}</span>`).join('')}
        </div>
      ` : ''}
      ...
      <div class="pdf-footer">
        Generated by GRID — Grade Research & Interactive Diary
      </div>
    `;
