...
```mermaid
graph TD
    A[Structure of the Atom] --> B[Sub-atomic Particles]
    A --> C[Atomic Models]
    A --> D[Atomic Number & Mass Number]
    A --> E[Electron Distribution]
    A --> F[Isotopes & Isobars]
    B --> B1[Electron: negative charge]
    B --> B2[Proton: positive charge]
    B --> B3[Neutron: no charge]
    C --> C1[Thomson's Model]
    C --> C2[Rutherford's Model]
    C --> C3[Bohr's Model]
    D --> D1[Atomic Number Z = p]
    D --> D2[Mass Number A = p + n]
    E --> E1[Shells: K L M N]
    E --> E2[Maximum electrons = 2n²]
    F --> F1[Isotopes: same Z, different A]
    F --> F2[Isobars: same A, different Z]
```
